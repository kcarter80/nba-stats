...
data['resultSets']['rowSet'].each_with_index do |player,i|
  ...
  paintPercent = paintMakes > 1 ? paintMakes.to_f / paintAttempts : nil

  midMakes = clean(player[11])
  midAttempts = clean(player[12])
  midPercent = midMakes > 1 ? midMakes.to_f / midAttempts : nil

  threeAttempts = clean(player[15])+clean(player[18])+clean(player[21])
  threeMakes = clean(player[14])+clean(player[17])+clean(player[20])
  threePercent = threeMakes > 1 ? threeMakes.to_f / threeAttempts : nil

  # (FG + 0.5 * 3P) / FGA
  if midAttempts > 0 || threeAttempts > 0
  	outsidePaintEPercent = (midMakes + threeMakes * 1.5).to_f / (midAttempts + threeAttempts) 
  else
...
massagedData.reject! { |player| player[5] + player[8] < 300 }
